...
**Do you need a "Worker"?** No. Some projects (like the SA CPI dashboard) need a small
Cloudflare Worker running in the cloud because their data source (StatsSA) blocks browsers
from fetching it directly. **The SARB API has no such block** — it sends the CORS header
(`Access-Control-Allow-Origin: *`) that tells browsers "anyone may read this". So the SARB
dashboard is pure static hosting: no Worker, no cron trigger, no KV storage, no secrets.
